...
<nav
    ...
    <h1><u>Age Calcualtor</u></h1>
<div class="cal">
<label>Any Date: </label>
      <input type ="date" v-model="number" placeholder="Enter Numbers" style="width:10%;box-border:2px;" /><br><br>
<label>Date of Birth: </label>
      <input type ="date" v-model="number1" placeholder="Enter Numbers" /><br><br>
      <button v-on:click="click">Calculate</button>
      <p>Result: {{res}}</p>
</div>
</template>
...
export default {
  ...
  methods:{
      click(){
      this.today = new Date(this.number);
    this.birthDate = new Date(this.number1);
    this.res = this.today.getFullYear() - this.birthDate.getFullYear();
