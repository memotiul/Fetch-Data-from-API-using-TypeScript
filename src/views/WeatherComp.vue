...
<div class="hello">
    <h1><u>Weather App-Composition API</u></h1>
</div>
 <form @submit.prevent="getWeather">
    <div >
      <label>City: </label>
      <input type="text" v-model="citySearch" />
    </div><br>
          <button type="submit">get weather</button>
  </form>
  <div v-if="weather.main !== undefined">
  <p>Country Name: {{ weather.sys.country }}</p>
...
const weather = ref({});
const apiKey= "dbce4955f4db4d1d36d7f7dcd016a92f";
const apiLink = ref("");

const getWeather=async()=> {
  apiLink.value = `https://api.openweathermap.org/data/2.5/weather?q=${citySearch.value}&appid=${apiKey}&units=metric`;
    const response = await axios.get(apiLink.value);
