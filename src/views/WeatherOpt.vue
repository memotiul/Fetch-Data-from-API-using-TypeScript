...
<div class="hello">
    <h1><u>Weather App-Option API</u></h1>
</div>

 <form @submit.prevent="getWeather">
    <div>
      <label>City: </label>
      <input v-model.trim="citySearch" />
    </div><br>
    <button type="submit">get weather</button>
  </form>
  <div>
  <p>City Name: {{ weather.cityName }}</p>
  <p>Country: {{ weather.country}}</p>
  <p>Description: {{ weather.description }}</p>
  <p>Feellike: {{ weather.feelsLike }}</p>
  <p>Humidity: {{ weather.humidity }}</p>
  <p>Temperature: {{ weather.temperature }}</p>
  <p>Low Temperature: {{ weather.lowTemp}}</p>
  <p>High Temperature: {{ weather.highTemp }}</p>
  </div>
</template>
...
export default {
 name: 'OptionCal',
  ...
  methods: {
      async  getWeather () {
      const key = 'dbce4955f4db4d1d36d7f7dcd016a92f';
